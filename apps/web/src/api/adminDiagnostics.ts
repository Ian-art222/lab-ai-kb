...
export async function getDiagnosticsTracesApi(params: Record<string, unknown>): Promise<TraceListResponse> {
  ...
  const response = await apiFetch(`/admin/diagnostics/traces?${q.toString()}`)
  return readJsonOk<TraceListResponse>(response, '加载诊断列表失败')
}

export async function getDiagnosticsTraceDetailApi(traceId: string): Promise<TraceDetail> {
  const response = await apiFetch(`/admin/diagnostics/traces/${encodeURIComponent(traceId)}`)
  return readJsonOk<TraceDetail>(response, '加载 Trace 详情失败')
}

export async function retryIndexFileApi(fileId: number, forceReindex = false): Promise<void> {
  const response = await apiFetch(
    `/admin/diagnostics/files/${fileId}/retry-index?force_reindex=${forceReindex ? 'true' : 'false'}`,
    { method: 'POST' },
  )
  await readJsonOk<Record<string, unknown>>(response, '重试索引失败')
}

export async function getReasonStatsApi(): Promise<ReasonStatsItem[]> {
  const response = await apiFetch('/admin/diagnostics/traces/stats/reasons')
  return readJsonOk<ReasonStatsItem[]>(response, '加载 reason 统计失败')
}

export async function exportTraceApi(traceId: string): Promise<Record<string, unknown>> {
  const response = await apiFetch(`/admin/diagnostics/traces/${encodeURIComponent(traceId)}/export`)
  return readJsonOk<Record<string, unknown>>(response, '导出 trace 失败')
}
