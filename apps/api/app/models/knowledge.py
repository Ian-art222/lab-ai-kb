...
class QARetrievalTrace(Base):
    ...
    normalized_query: Mapped[str | None] = mapped_column(sa.Text, nullable=True)
    rewritten_queries_json: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    ...
    retrieval_strategy: Mapped[str | None] = mapped_column(sa.String(80), nullable=True)
    filters_json: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    selected_evidence_json: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    evidence_bundles_json: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    strict_mode: Mapped[bool | None] = mapped_column(sa.Boolean, nullable=True)
    is_abstained: Mapped[bool] = mapped_column(sa.Boolean, nullable=False, default=False)
    failed: Mapped[bool] = mapped_column(sa.Boolean, nullable=False, default=False)
    ...
    latency_breakdown_json: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    task_type: Mapped[str | None] = mapped_column(sa.String(50), nullable=True)
    tool_traces_json: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    workflow_steps_json: Mapped[list | None] = mapped_column(JSONB, nullable=True)
