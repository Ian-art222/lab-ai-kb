...
_STYLE_BY_QUERY_TYPE: dict[str, str] = {
    QUERY_TYPE_FACTUAL: (
        "【回答结构-事实型】\n"
        "1) 先用一两句话直接回答核心问题。\n"
...
def _style_for_query_type(query_type: str) -> str:
    """未知 query_type 时回退到事实型样式；禁止递归 .get（曾导致无限递归）。"""
    key = (query_type or "").strip()
    if key in _STYLE_BY_QUERY_TYPE:
        return _STYLE_BY_QUERY_TYPE[key]
    return _STYLE_BY_QUERY_TYPE[QUERY_TYPE_FACTUAL]
