...
def upgrade() -> None:
    ...
    op.add_column("qa_retrieval_traces", sa.Column("normalized_query", sa.Text(), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("rewritten_queries_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("retrieval_strategy", sa.String(length=80), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("filters_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("selected_evidence_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("evidence_bundles_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("strict_mode", sa.Boolean(), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("is_abstained", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    op.add_column("qa_retrieval_traces", sa.Column("failed", sa.Boolean(), nullable=False, server_default=sa.text("false")))
    ...
    op.add_column("qa_retrieval_traces", sa.Column("latency_ms", sa.Integer(), nullable=True))
    op.add_column("qa_retrieval_traces", sa.Column("latency_breakdown_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    ...
    op.execute("UPDATE qa_retrieval_traces SET trace_id = md5(random()::text || clock_timestamp()::text) WHERE trace_id IS NULL")
    op.alter_column("qa_retrieval_traces", "trace_id", nullable=False)

    op.create_index("ix_qa_retrieval_traces_trace_id", "qa_retrieval_traces", ["trace_id"], unique=True)
    op.create_index("ix_qa_retrieval_traces_request_id", "qa_retrieval_traces", ["request_id"], unique=False)
    op.alter_column("qa_retrieval_traces", "is_abstained", server_default=None)
    op.alter_column("qa_retrieval_traces", "failed", server_default=None)


def downgrade() -> None:
    op.drop_index("ix_qa_retrieval_traces_request_id", table_name="qa_retrieval_traces")
    op.drop_index("ix_qa_retrieval_traces_trace_id", table_name="qa_retrieval_traces")
