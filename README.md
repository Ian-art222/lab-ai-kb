...
在不强制“每次都多文档引用”的前提下，QA 组装阶段新增了文档感知与去冗余策略：

- `QA_MAX_CHUNKS_PER_DOC`：单文档最多进入上下文的 chunk 数。
- `QA_TARGET_DISTINCT_DOCS`：当存在互补证据时，目标覆盖的文档数。
- `QA_MIN_DISTINCT_DOCS_FOR_MULTI_SOURCE`：低于该文档数时不强行做多源覆盖。
- `QA_SINGLE_DOC_DOMINANCE_RATIO`：若第一文档显著强于第二文档，允许保持单文档主导。
- `QA_DIVERSITY_RERANK_ENABLED` / `QA_DIVERSITY_LAMBDA`：轻量相关性+多样性重排。
- `QA_MERGE_ADJACENT_GAP` / `QA_REDUNDANCY_SIM_THRESHOLD`：抑制相邻/高重复 chunk 占满上下文。

该策略目标是：单文档足够时保持稳定，多文档有增量证据时自然融合，减少同文档相邻片段重复堆叠。
